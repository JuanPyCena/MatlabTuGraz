...
while t <= stop
    ...
    not_x =  [grav_potent(1,t), grav_potent(2,t), grav_potent(3,t), grav_potent(4,t), grav_potent(5,t)]';
    
    dh = h(:,t+1) - h(:,t);
    
    for i = 1:numel(x)
        delta_x_fun    = zeros(numel(x), 1);
        delta_x_fun(i) = delta_x(i);
        [T_g_plus_delta_x, w_plus_delta_x(:,i)] = calculate_w(x, not_x, dt, w_in, T_g, h(:,t), r_sun, r_moon, dh, coefficient_F,coefficient_T_g, coefficient_T_r, GM_sun, GM_moon, delta_x_fun);
    end
    [T_g, w_0]                         = calculate_w(x, not_x, dt, w_in, T_g, h(:,t), r_sun, r_moon, dh, coefficient_F,coefficient_T_g, coefficient_T_r, GM_sun, GM_moon, zero_delta);
      
    A_matrix = getA(w_plus_delta_x, w_0, delta_x);
    
    l_0     = w_0;
    l       = reference(:,t);
    delta_l = l - l_0;

    delta_x_dach = ((A_matrix' * A_matrix)\A_matrix') * delta_l;
    x_dach       = x + delta_x_dach;
    
    dif_x = x_dach- x;
    [T_g, w_new] = calculate_w(x_dach, not_x, dt, w_in, T_g, h(:,t), r_sun, r_moon, dh, coefficient_F,coefficient_T_g, coefficient_T_r, GM_sun, GM_moon, delta_x);
    
    % relativ difference to the previous iteration
    diff = abs((w_new-reference(:,t)))
